...
open class JCChatViewCell: UICollectionViewCell, UIGestureRecognizerDelegate {
    ...
    func updateView() {
        ...
        if message.contentType == .image {
            _bubbleView?.isHidden = true
        } else {
            _bubbleView?.isHidden = false
        }
    ...
    private func _updateViews() {
        ...
        if options.showsBubble {
            if _bubbleView == nil {
                ...
                _bubbleView?.backgroundColor = ViewControllerLightGray()
                _bubbleView?.clipsToBounds = true
                _bubbleView?.cornerRadius = 3
            }
...
//                view.highlightedImage = send_press
                view.backgroundColor = RGBAColor(1, 245, 93, 1)
            case .center:
                break
            }
        }
...
fileprivate extension JCMessageContentViewType {
    ...
    static func _init() -> JCMessageContentViewType {
...
fileprivate extension UICollectionReusableView {
    @NSManaged func _setLayoutAttributes(_ layoutAttributes: UICollectionViewLayoutAttributes)
}
